...
## 백로그 (2026-08-15 태찬 제안)
- **Supabase → 오라클 서버 이전 검토**: 정적 프론트는 caddy 서빙, DB는 서버 Postgres/SQLite + 경량 API로 대체. 난이도 반나절~하루. 킵얼라이브(매일)로 당장 리스크 0이라 **필기(8/22) 이후 '합격 기념 업글' 때 진행** — keep-system-simple 원칙, 친구들(지영·시연) URL 전환 공지 필요.
